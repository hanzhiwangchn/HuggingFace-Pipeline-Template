...
def build_parser():
    ...
    # dataset settings
    parser.add_argument('--dataset-dir', type=str, default='../TestBed/ImageFolder',
    ...
    parser.add_argument('--val-test-size', type=float, default=0.9,
    ...
    parser.add_argument('--num-train-epochs', type=int, default=10, help='number of epoch')
    # optimizer settings
    ...
    parser.add_argument('--run-code-test', action='store_true', default=False,
                        help='test pipeline functionality')
    # default settings
...
def main():
    ...
    dataset_train, dataset_val, dataset_test, label2id, id2label = build_datasets(args=args)
    logger.info('Dataset loaded')

    # build image processor 
    dataset_train, dataset_val, dataset_test, image_processor = preprocess_datasets(dataset_train=dataset_train, 
        dataset_val=dataset_val, dataset_test=dataset_test, args=args)
    logger.info('Dataset preprocessed')

    # build models
    # In the training phase, we usually load pretrained models and fine-tune it. 
    # In the testing phase, we need to load model with best performance locally.
    ...
    if args.use_trainer:
        ...
        # Train models using native Pytorch loop
