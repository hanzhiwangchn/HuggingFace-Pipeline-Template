...
def build_data_loader(args, dataset_train, dataset_val, dataset_test):
    ...
    val_kwargs = {'batch_size': args.batch_size, 'shuffle': False, 'collate_fn': collate_fn}
...
def validation(model, val_loader, device, m):
    all_metrics = dict()
...
def test(model, test_loader, device, m, args, label2id, id2label):
    ...
    # When we do training, we need to load model with best accuracy for testing.
